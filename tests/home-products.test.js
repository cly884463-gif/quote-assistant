...
const tn304 = cards.find((product) => product.model === "TN-304");
...
assert.deepStrictEqual(yc231.specRows.map((row) => row.dealerPrice), [406, 768, 2498]);
assert.deepStrictEqual(yc231.specRows.map((row) => row.channelPrice), [508, 960, 3123]);
assert.ok(yc232);
assert.deepStrictEqual(yc232.specRows.map((row) => row.dealerPrice), [358, 704, 2106]);
assert.deepStrictEqual(yc232.specRows.map((row) => row.channelPrice), [448, 880, 2633]);
assert.ok(tn304);
assert.deepStrictEqual(tn304.specRows.map((row) => row.dealerPrice), [100, 426]);
assert.deepStrictEqual(tn304.specRows.map((row) => row.channelPrice), [125, 533]);
assert.ok(qc401);
...
assert.strictEqual(hColorPaste.category, "H系列色浆");
